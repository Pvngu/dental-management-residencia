...
class UpdateRequest extends BaseRequest
{
    ...
    public function withValidator(Validator $validator)
    {
        $validator->after(function ($validator) {
            ...
            $purchaseId = $this->route('purchase_medicine');

            // Get existing order medicine IDs for this purchase
            $existingOrderMedicineIds = [];
            if ($purchaseId) {
                $purchaseXid = $this->getIdFromHash($purchaseId);
                $existingOrderMedicineIds = OrderMedicine::where('purchase_medicine_id', $purchaseXid)
                    ->pluck('id')
                    ->toArray();
            }
            ...
            foreach ($orderMedicines as $index => $medicine) {
                ...
                if ($medicineId && $lotNo) {
                    // Check if this batch already exists (excluding current purchase medicines)
                    $query = OrderMedicine::where('medicine_id', $medicineId)
                        ->where('lot_no', $lotNo)
                        ->where('company_id', $company->id);

                    if (!empty($existingOrderMedicineIds)) {
                        $query->whereNotIn('id', $existingOrderMedicineIds);
                    }
