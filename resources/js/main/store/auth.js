...
export default {
    ...
    state() {
        return {
            ...
            googleMapsApiKey: window.localStorage.getItem(GOOGLE_MAPS_API_KEY) || '',
        }
    },
    ...
    mutations: {
        updateUser(state, user) {
            ...
            // Update language from user preference if available
            if (user && user.language) {
                state.lang = user.language;
                window.localStorage.setItem(SELECTED_LANG, user.language);
            }
        },
        ...
        updateToken(state, token) {
            ...
            // Update Echo auth headers when token changes
        ...
        updateGoogleMapsApiKey(state, apiKey) {
            state.googleMapsApiKey = apiKey;
            window.localStorage.setItem(GOOGLE_MAPS_API_KEY, apiKey);
        },
    },
        ...
        updateApp(context) {
            return axiosAdmin.get('/app')
                .then(function (response) {
                    ...
                    context.commit('updateGoogleMapsApiKey', response.data.google_maps_api_key);
